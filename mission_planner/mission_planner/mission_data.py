...
class MissionDatabase:
    ...
    def execute_row(self, task, navigation_client, scan_client):
        for point in task.get("scan_points", []):
            ...
            success = scan_client.perform_scan(task.get("table_id"), task.get("row_id"), point["name"])
            if not success:
                self._node.get_logger().error(f"Failed to perform scan at {point}, skipping to next point.")
                break
        if success:
            self.mark_completed(table_id=task.get("table_id"), row_id=task.get("row_id"))
            self._node.get_logger().info(f"Completed task for table {task.get('table_id')} row {task.get('row_id')}.")