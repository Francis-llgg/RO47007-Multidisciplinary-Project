...
class MappingManagerNode(Node):
    """
    Mapping manager for the greenhouse robot.

    This node does not implement SLAM itself.
    It monitors the required mapping topics and provides a service to save the map.
    ...
    SLAM is provided by slam_toolbox.
    Map saving is handled by nav2_map_server map_saver_cli.
    """
    ...
    def save_map_callback(self, request, response):
        ...
        if self.map_save_dir == "":
            ...
            self.map_save_dir = os.path.join(package_dir, "maps")
        ...
        self.get_logger().info(f"Saving map to: {map_base_path}.yaml / .pgm")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                response.message = f"Map saved successfully: {map_base_path}.yaml"
